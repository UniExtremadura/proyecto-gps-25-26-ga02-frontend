...
function LabelStatsDashboard() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                        ...
                        if (items && items.length) {
                            ...
                            if (hasArtistIds) {
                                const mapped = items.map(it => ({
                                    ...
                                    ratings_count: it.ratings_count || it.count || 0,
                                    ratings_average: typeof it.ratings_average !== 'undefined' ? it.ratings_average : (typeof it.average !== 'undefined' ? it.average : null),
                                }));
                ...
                // and aggregating per-track rating aggregates. This is used as a
                // fallback when the stats service doesn't return artist-linked rows.
                const computeRatingsForArtist = async (art) => {
                    ...
                    try {
                        ...
                        for (const tr of items) {
                            const tid = tr.id || tr.track_id || tr.uuid || tr.song_id || tr.title;
                            if (!tid) continue;
                            try {
                                ...
                                let count = 0;
                                if (rb == null) { continue; }
                                else if (typeof rb.count !== 'undefined' && typeof rb.average !== 'undefined') {
                                    count = Number(rb.count) || 0;
                                    avg = count ? Number(rb.average) : null;
                                } else if (Array.isArray(rb.results) && rb.results.length) {
                                    const arr = rb.results;
                                    count = arr.length;
                                    const sum = arr.reduce((s, it) => s + (Number(it.stars) || 0), 0);
                                    avg = count ? (sum / count) : null;
                                } else if (Array.isArray(rb.items) && rb.items.length) {
                                    const arr = rb.items;
                                    count = arr.length;
                                    const sum = arr.reduce((s, it) => s + (Number(it.stars) || 0), 0);
                                    avg = count ? (sum / count) : null;
                                } else if (Array.isArray(rb) && rb.length) {
                                    const arr = rb;
                                    count = arr.length;
                                    const sum = arr.reduce((s, it) => s + (Number(it.stars) || 0), 0);
                                    avg = count ? (sum / count) : null;
                                } else if (rb && typeof rb.stars !== 'undefined') {
                                    avg = Number(rb.stars) || null;
                                    count = 1;
                                }
                                if (count > 0 && avg !== null) {
                                    totalCount += count;
                                ...
                                // ignore track-level failures
                                continue;
                            }
                        }
                        const artistAvg = totalCount > 0 ? (weightedSum / totalCount) : null;
    ...
    // Re-run the load effect when reloadKey changes
    useEffect(() => {
        const load = async () => {
            try {
                setLoading(true);
                setError(null);
                // Reuse the same logic by delegating to the existing effect body
                // We'll just call the first effect's work by programmatically
                // fetching the same endpoint(s) here. Simpler than refactoring.
                const rEnriched = await fetch(`${API_STATS_BASE}/stats/artists/aggregate?limit=1000&sort=average&enrich=1`, { headers: ROLE_HEADER });
                if (rEnriched.ok) {
                    const body = await rEnriched.json();
                    const items = body.items || [];
                ...
                // Fallback to canonical compute if enriched not available
                const rArtists = await fetch(`${API_CONTENT_BASE}/artists/`);
                if (!rArtists.ok) throw new Error(`Error fetching artists ${rArtists.status}`);
                const artistsResp = await rArtists.json();
                let artistsArr = [];
