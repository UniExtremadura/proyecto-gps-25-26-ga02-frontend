...
export default function Ratings() {
    ...
    const [expanded, setExpanded] = useState({}) // artistId -> bool
    const [songsMap, setSongsMap] = useState({}) // artistId -> [songs]
    const [ratingsMap, setRatingsMap] = useState({}) // songId -> {average, count, user_rating}
    const [artistAggregates, setArtistAggregates] = useState({}) // artistId -> {average, count, _loading}
    const [selectedValues, setSelectedValues] = useState({}) // songId -> editable value
    const [saving, setSaving] = useState({})
    const [errorMessages, setErrorMessages] = useState({}) // songId -> error message

    // Helpers: centralize JWT decoding/validation used across functions
    const isJwtValid = (t) => {
    ...
    useEffect(() => {
        // Entry guard: only regular users should access Ratings page.
        let mounted = true
        ;(async () => {
            try {
                const role = await getCurrentUserRole()
                const r = role ? String(role).toLowerCase() : ''
                if (r && (r.includes('artist') || r.includes('discograf') || r.includes('label'))) {
                    window.location.href = '/'
                    return
                }
            } catch (e) {
                // If we cannot determine role, be conservative and allow access
                // only when role is explicitly a user. Otherwise redirect.
            }
        })()
        ...
        const load = async () => {
            try {
                ...
                if (!cancelled) {
                    ...
                    // Restore expanded artists from localStorage so page reload preserves state
                    ...
                    for (const art of mapped) {
                        if (toExpand.includes(art.id)) expandObj[art.id] = true
                    }
                    setExpanded(expandObj)
                    // Preload songs and per-song aggregates for every artist so
                    // the page shows song averages and counts immediately on
                    // first visit (the user requested aggregates be visible
                    // as soon as the page loads). This may increase network
                    // activity but ensures the UI is ready without interaction.
                    for (const art of mapped) {
                        try {
                            // fetch songs for artist and store them (avoid double-fetch)
                            const items = await fetchArtistSongs(art.id)
                            if (items && items.length) {
                                setSongsMap(prev => ({ ...prev, [art.id]: items }))
                                // initialize placeholders for ratings
                                setRatingsMap(prev => {
                                    const copy = { ...prev }
                                ...
                                // kick off per-song aggregate fetches but don't await each
                                for (const s of items) {
                                    const sid = s.id || s.song_id || s.track_id || s.title
                            ...
                            // Ignore individual artist failures and continue
                            console.warn('Preloading songs failed for', art.id, e)
                        }
                        // Also fetch server-side artist aggregate for header
                        try { void fetchArtistAggregate(art.id) } catch (e) { /* ignore */ }
                    }
                }
            } catch (err) {
    ...
    const ratingActiveRef = React.useRef(0)
    const ratingQueueRef = React.useRef([])
    ...
    // Fetch artist aggregates proactively (without expanding UI). This will
    // fetch the artist's songs and schedule per-song rating fetches so the
    // header aggregate is visible immediately on page load.
        ...
        try {
            ...
            if (resp && resp.ok) {
                try {
                    ...
                    // Do not return early if we already have songs loaded and need to compute per-song details.
                    // But if songs are not loaded, we can avoid fetching per-track lists now.
                    const existing = songsMap[artistId]
                    if (existing && existing.length) {
                        // ensure per-song aggregates are consistent
